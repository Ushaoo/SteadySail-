...
// LEDC 操作互斥锁
static SemaphoreHandle_t ledc_mutex = NULL;
...
// 设置特定通道的 PWM 脉冲 (us) - 简化版，Arduino风格
static void set_pwm_us(ledc_channel_t channel, uint32_t us) {
    ...
    // 获取互斥锁保护LEDC操作
    if (xSemaphoreTake(ledc_mutex, pdMS_TO_TICKS(100)) != pdTRUE) {
        ESP_LOGW(TAG, "Failed to acquire LEDC mutex for CH%d", channel);
        return;
    }
    ...
    ESP_LOGI(TAG, "  CH%d: %d us -> duty %d", channel, us, duty);
    
    xSemaphoreGive(ledc_mutex);
    
    // 在锁外等待硬件完成一个PWM周期
    // 低速模式需要完整周期（20ms @ 50Hz）才能生效
