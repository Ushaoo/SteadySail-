...
static float compute_angle(volatile uint32_t high_us, volatile uint32_t period_us, volatile bool valid) {
    ...
    float duty = ((float)high_us / (float)period_us) * 4119.0f - 16.0f;
...
void steering_control_get_current_angles(float *left_deg, float *right_deg) {
    *left_deg = compute_angle(enc_left.high_us, enc_left.period_us, enc_left.valid);
    *right_deg = compute_angle(enc_right.high_us, enc_right.period_us, enc_right.valid);
}
...
static float calculate_pid(float error, float *integral, float *prev_error, float *out_filt) {
    ...
    *integral += err_smooth * dt;
    ...
    float derivative = (err_smooth - *prev_error) / dt;
    *prev_error = err_smooth;
