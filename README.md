...
说明：`raspberry_pi_backup/` 不属于当前 ESP-IDF 编译链路，但目录内保留了上位机调参、数据分析、可视化和旧控制实验脚本，本文末尾补了用途索引，便于后续查找。

---

## 1. 项目概览

SteadySail 是一个基于 ESP32-S3 的双推进器 + 双矢量舵机姿态稳定控制系统，当前实际应用场景是安装在桨板（SUP）上的电动辅助稳定平台，目标是在桨板低速巡航、原地等待和受扰动工况下实现横滚稳定与航向保持。当前实现采用：
...
核心目标：在给定前进推力的同时维持桨板板体横滚稳定，并允许通过 App / 串口 / RC 遥控实时干预。
...
- `main/CMakeLists.txt`：注册以下源文件，依赖 `Blinker json nvs_flash`
  ...
  - `gps_nmea.c`、`fusion.c`、`anchor_control.c`

### 主功能模块（`main/`）

- [main.c](main/main.c)
  - 系统初始化、按模式分发任务
  - 100 Hz `control_core_task`：融合更新 + 虚拟锚点接管 + V3 矢量控制 + H=0 静止路径 + 差速转向 + 航向保持 + 安全保护
  - RC 输入读取（`rc_input_get_throttle()`），定速巡航激活时同步锁定航向
  - 串口监测输出改为 `ESP_LOG*`，并通过 `MAIN_PRINT_INTERVAL` 控制主循环打印频率
  - 串口命令解析（`w/s/space/数字/r/cal/reboot`）
- [system_config.h](main/system_config.h)
  - 运行模式、转向控制模式
  - GPIO 映射（I2C/SPI/PWM 输入输出/RC 输入/GPS/锚点开关）
  - 主循环日志节流参数（`MAIN_PRINT_INTERVAL`）
  - PWM 安全限幅、方向反转开关、目标角镜像开关
  - 差速转向参数（`TURN_DELTA_H`、`TURN_MIN_FWD_PCT`）
  - 航向保持 PID 参数（`HEADING_KP`、`HEADING_KI`）
  - 虚拟锚点参数（内外圈半径、返航推力、GPS 超时、开关去抖）
  - 安全保护与控制基础参数
...
- [steering_control.c](main/steering_control.c) / [.h](main/steering_control.h)
  ...
  - SPI 读包增加 CRC-8 校验；并用互斥锁避免控制任务与原始抓包任务并发访问总线
...
- [rc_input.c](main/rc_input.c) / [.h](main/rc_input.h)
  ...
  - 定速取消增加 3 帧去抖，降低单帧噪声导致的误取消
  - 提供 `rc_input_get_raw_pwm()`，便于主循环日志直接显示当前 RC 原始脉宽
...
- [blinker_bridge.c](main/blinker_bridge.c) / [.h](main/blinker_bridge.h)
  ...
  - 2 s 周期上报 PWM、舵角、Roll、PID 数值和急停状态
- [gps_nmea.c](main/gps_nmea.c) / [.h](main/gps_nmea.h)
  - UART1 NMEA-0183 驱动（9600 8N1，GPIO 38/39）
  - 解析 `GPRMC/GNRMC` 与 `GPGGA/GNGGA`，输出经纬度、地速、卫星质量信息
  - 每帧同时喂给 `anchor_control` 与 `fusion`
- [fusion.c](main/fusion.c) / [.h](main/fusion.h)
  - GPS + BNO055 传感器融合定位：ENU 航位推算 + GPS 互补校正
  - 利用 GPS COG 慢学习 BNO055 航向偏置，并在静止时执行 ZUPT 零速约束
  - 提供融合后的经纬度、航向、速度向量、局部 ENU 查询接口
- [anchor_control.c](main/anchor_control.c) / [.h](main/anchor_control.h)
  - 虚拟锚点状态机：`SETTING` / `HOLDING` / `RETURNING` / `LOST_GPS`
  - 抛锚时取多帧 GPS 平均确定锚点，回点时自动选择板头/板尾最短转角方案
  - 接管 `g_forward_thrust`、`g_target_heading`、`g_heading_hold_active`
...
| `MODE_TEST_ENCODER_SPI` | 6 | 在控制功能保持可用的同时，每 100 ms 输出两路编码器 SPI 原始 6 字节数据 |
...
| GPS UART RX / TX | GPIO 38 / 39 |
| 虚拟锚点物理开关 | GPIO 40（内部上拉，LOW=抛锚） |
...
| `STEER_LEFT_INVERT` / `STEER_RIGHT_INVERT` | 0 / 0 | 舵机 PWM 镜像 |
...
1. **RC 输入**：`rc_input_get_throttle()` 更新 `g_forward_thrust`；检测定速巡航状态，激活时同步锁定 BNO055 当前偏航角（`g_heading_hold_active = true`）
  - 调试日志会同时显示 `rc_input_get_raw_pwm()` 返回的原始 RC 脉宽，便于排查接收机抖动
2. **锚点/融合更新**：`fusion_update()` 先吸收 GPS 校正并做 DR，随后 `anchor_control_update()` 按需接管推力与航向
3. 读取舵机当前角度（MT6826S SPI 编码器）
4. **BNO055 路径**：`balance_controller_update()` 直接从 BNO055 读取 Roll + GyroX → PID → `tau_total`
5. 安全保护：`|roll| > 60°` → 紧急停推 + 舵机回 180°；**磁控急停**（`g_hard_estop = true`）→ 全通道锁 1500 µs，等待磁铁重新吸合 500 ms 后自动重启
6. **H=0 静止路径**：`|H_thrust| < 5`（无前进推力）→ 舵机锁 180°，双电机正反转产生上下力差
7. **V3 矢量分解**（详见 §8）→ `target_angle_L/R` + `T_thrust_L/R`
8. **差速转向 / 航向保持**：`g_turn_state` 或 BNO055 偏航 PI 修正 `H_L / H_R`
9. 目标角线性低通（`filter_target += 0.4 * Δ`，结果 clamp 到 [80°, 280°]，不跨 0/360 边界）
10. **L/R 物理输出交换**：`send_tgt_L = 360 − filter_target_R`
11. **方案 A 推力门控**：按舵机就位误差限制推力上升斜率，避免大角度切换时先给满推力
12. 编码器掉线时立即把两路舵机与两路推进 PWM 全部压回 1500 µs
13. `motor_control_set_pwm_bidirectional()` 下发
14. `vTaskDelayUntil()` 保持 10 ms 周期
...
### 7.4 GPS + BNO055 融合定位（fusion.c）

- 首帧有效 GPS fix 建立参考原点，后续用 ENU 坐标维护相对位置
- 100 Hz 读取 BNO055 线性加速度（LIA）与航向，执行 body→ENU 旋转后积分速度与位置
- 每次 GPS 到来时吸收位置误差（互补滤波）并校正速度向量
- 当 GPS 位移与地速足够可信时，用 COG 慢学习 `s_psi_bias_deg`，修正 BNO055 绝对航向常值偏差
- 当 GPS 低速且线加速度接近 0 时触发 ZUPT：清零速度并慢学习加速度计零偏

### 7.5 虚拟锚点（anchor_control.c）

- 入口由 GPIO 40 物理开关或软件调用 `anchor_set_here()` / `anchor_release()` 触发
- `SETTING`：采样 `ANCHOR_SETTLE_SAMPLES` 个 GPS fix 求平均，确定锚点
- `HOLDING`：距离落在内圈内时令 `g_forward_thrust = 0`，只漂浮不返航
- `RETURNING`：超出外圈后自动比较板头/板尾朝向锚点的最短转角，决定前进或倒车回到锚点附近
- `LOST_GPS`：融合无效或 GPS 超时后停推，并等待 GPS 恢复再回到 `HOLDING`
...
- 推力只对上升沿做限速，下降沿直通，避免舵机大角度转动时推进器先推满
- 左右两侧分别按舵机误差算出门控比例，再取较小值做对称门控，避免额外偏航
- 这部分逻辑直接位于 [main.c](main/main.c) `control_core_task()` 内，依赖 `steering_control_get_target()` 与实时编码器角度
...
- 该路径支持桨板**近静止/原地等待时**的横滚稳定，无需前进推力
...
- **MT6826S SPI 模式**（`ENC_USE_SPI = 1`）：两路编码器共用 MISO/MOSI/SCLK，各独立 CS
  ...
  - 收包后对 `rx[2..4]` 执行 CRC-8 校验，CRC 不通过则本帧判无效并记录告警
  - 抗 EMI 能力优于 PWM 捕获模式
- 角度 clamp + `ENC_*_REVERSE` 开关 + NVS 零点偏移补偿
...
- SPI 模式下采用双阈值去抖：连续 20 帧失败才判故障，连续 5 帧成功才恢复有效
- 无效帧期间 `steering_control_get_current_angles()` 会冻结上一帧有效角度，避免 PID 追逐瞬时脏数据

### 9.2.1 编码器 SPI 原始抓包模式

- `MODE_TEST_ENCODER_SPI` 下会额外创建 `encoder_spi_raw_print_task()`，每 100 ms 打印左右编码器的原始 6 字节响应、状态位、15-bit 原始角度值和换算角度
- 该模式下 `control_core_task()` 仍正常运行，适合在保持整机联调的同时定位 SPI 通讯问题
- `steering_control_spi_read_raw()` 对外暴露原始读接口，供该模式直接调用

### 9.3 编码器掉线保护

- 任意一侧编码器失效时，主循环立即把两路推进和两路舵机全部锁到 1500 µs
- 同时清零推力门控历史，避免编码器恢复后的第一拍出现推力跳变
- 这层保护比 `steering_control_update()` 内部的单侧静音更激进，属于整机级 fail-safe
...
- `motor_control_init()` 在舵机通道初始化后，会主动发送一组唤醒/复位脉冲：2050 µs → 950 µs → 1500 µs
- 最后的 1500 µs 会保持约 1.5 s，用于让 360° 连续舵机锁定到“零速中立”状态
- 这一步属于上电流程的一部分，因此首次通电后看到舵机短暂动作是预期行为，不是控制环误动作
...
| `s` / `S` | 推力 −10% |
| `space` 或 ` ` | 推力归零 |
| 数字（如 `30` / `-50`） | 直接设置推力百分比（±50 限幅） |
| `r <角度>` | 演示模式下设置模拟 Roll（仅 `DEMO_MANUAL_ROLL=1`） |
| `reboot` | 延迟 1 s 软件重启 ESP32 |
...
### MODE_TEST_ENCODER_SPI 额外

- 每 100 ms 打印左右编码器原始 SPI 响应与解码结果
- 串口命令支持 `cal`、`reboot`、`w`、`s` 和直接输入推力数字
...
- **实时数据上报**（2 s）：推进 PWM、舵机实际角度、Roll、两组 PID 文本、6 个 PID 数值以及 `estop_state`
- 初始化阶段若尚未配网，`blinker_init()` 会等待 SmartConfig / Wi-Fi 连接
...
| 主循环打印频率 | `MAIN_PRINT_INTERVAL = 10`（100 Hz 主循环下约 10 Hz 输出） |
| IMU 配置 | BNO055 NDOF (`USE_BNO055_FOR_ROLL = 1`，`USE_DUAL_IMU = 0`) |
| 编码器模式 | SPI (`ENC_USE_SPI = 1`，2 MHz) |
| 控制频率 | 100 Hz (`CONTROL_DT = 0.01`) |
| 转向 PID 运行时默认值 | `g_steer_kp = 9.28`，`g_steer_ki = 6.9`，`g_steer_kd = 1.2` |
...
- [ ] 串口输入 `10` 后推进 PWM 在 1500–1800 µs 区间变化
...
| 现象 | 排查方向 |
|---|---|
| 桨板抖动 / 板体晃动 | 降 `PID_KP` 或 `FEEDBACK_PARAM` |
...
| 定速巡航容易被误取消 | 当前实现要求连续 3 帧非零输入才取消；若仍误触发，优先检查 RC 脉宽是否抖动严重 |
| 编码器偶发无效帧 | 优先看 SPI 线束、接地和 CRC mismatch 日志；CRC 失败会被直接判为无效数据 |
...
- ✅ 编码器 SPI 增加 CRC-8 校验 + 原始抓包测试模式，便于定位通讯问题
- ✅ RC 定速取消增加去抖，降低单帧噪声误取消
- ✅ 串口与 App 均支持在线调参与校准；其中 App 额外支持 `f` 推力文本命令
...
本 README 与当前仓库代码一一对应；`raspberry_pi_backup/` 仅作历史参考，不参与当前 ESP32-S3 固件编译。

---

## 18. raspberry_pi_backup 用途索引

该目录不在当前控制闭环中，但保留了前期 Raspberry Pi / PC 侧实验工具，主要分为以下几类：

- **串口/网络收发与在线控制**
  - `data_sender.py` / `data_receiver.py`：用于上位机与设备间的数据发送、接收与联调
  - `test_api.py`：接口联通性测试
  - `web_pid_tuner.py`：Flask Web 调参界面，支持 PID、前馈/反馈、质量/宽度、死区、推力缩放等参数在线管理与预设保存
- **IMU 采集、可视化与姿态实验**
  - `IMU_code.py`、`imu_data.py`、`dual_imu.py`：单/双 IMU 采样与融合实验
  - `imu_plotter.py`、`roll_angle_animation.py`：曲线绘图与横滚动画可视化
  - `sample_calibrate.py`：采样/校准辅助脚本
- **电机与执行机构实验**
  - `motor_control.py`、`motor_control2.py`、`motor_sync.py`、`motor_test.py`、`motor_music.py`：推进器控制、同步测试和功能性实验脚本
- **控制算法原型与离线分析**
  - `enhanced_pid.py`、`feedforward_dual_imu.py`、`feedforward_test.py`：旧版平衡控制与前馈策略原型
  - `system_stability_analyzer.py`：对 CSV 实验数据计算横滚、角速度、角加速度、频域和 PWM 同步指标
  - `stability_comparator.py`：对多组分析结果做对比汇总
  - `run_stability_analysis.py`：批量执行稳定性分析流程
- **配置文件**
  - `pid_presets.json`：Web 调参工具的预设参数文件

如果后续要整理历史实验链路，建议把该目录单独拆成 `tools/` 或 `research/` 文档，而不是继续把它视为“纯备份”。
